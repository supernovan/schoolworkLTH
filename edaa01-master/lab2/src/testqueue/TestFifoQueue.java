...
public class TestFifoQueue {
	...
	public final void testMakeQueueEmpty() {
	 ...
	 @Test
	  public final void testEmptyEmptyAppend() {
	    FifoQueue<Integer> secIntQueue = new FifoQueue<Integer>();
	    myIntQueue.append(secIntQueue);
	    assertEquals("Wrong size after empty append", 0, myIntQueue.size());
	  }
	 @Test
	  public final void testNotEmptyEmptyAppend() {
	    FifoQueue<Integer> secIntQueue = new FifoQueue<Integer>();
	    myIntQueue.offer(3);
	    myIntQueue.append(secIntQueue);
	    assertEquals("Wrong size after non-empty appended to empty", 1, myIntQueue.size());
	  }
	 @Test
	  public final void testNotEmptyNotEmptyAppend() {
	    FifoQueue<Integer> secIntQueue = new FifoQueue<Integer>();
	    myIntQueue.offer(3);
	    secIntQueue.offer(5);
	    secIntQueue.offer(2);
	    ...
	    secIntQueue.offer(8);
	    myIntQueue.append(secIntQueue);
	    assertEquals("Wrong size after non-empty appended to non-empty", 5, myIntQueue.size());
	  }
	 @Test
	 public final void testNotException() {
		myIntQueue.offer(5);
		myIntQueue.offer(2);
		myIntQueue.offer(2);
		myIntQueue.offer(7);

		Iterator test = myIntQueue.iterator();

		assertEquals("Wrong value from hasNext", test.hasNext(), true);
		assertEquals("Wrong value from next", test.next(), 5);
		assertEquals("Wrong value from next", test.next(), 2);
		assertEquals("Wrong value from next", test.next(), 2);
		assertEquals("Wrong value from next", test.next(), 7);
		assertEquals("Wrong value from hasNext", test.hasNext(), false);
		 
	 }
	 @Test(expected=NoSuchElementException.class)
	 public final void testErrpr() {
		 Iterator test = myIntQueue.iterator();
		 test.next();
	 }
